...
import useLayoutElements from "@/utils/dagreLayout";
...
const GraphMenuSidebar = () => {
  ...
  useEffect(() => {
    const open = JSON.parse(
      localStorage.getItem("GraphMenuSidebarOpen") ?? "false",
    );
    setOpen(open);
  }, []);
  ...
  useEffect(() => {
    ...
    const filteredNodes = nodes.filter((node) => {
      ...
      return (
        sceneName
          .toLocaleUpperCase()
          .indexOf(searchValue.toLocaleUpperCase()) !== -1
      );
    });

    if (!filteredNodes.length) return;

    const focusNode = filteredNodes[0];
    const zoom = 0.7;
        ...
        <Box>
          ...
          <MenuList sx={{ p: 0, justifyContent: "center" }}>
            ...
            <Submenu
              ...
              itemProps={{
                handlers: {
                  onClick: (event: React.SyntheticEvent, index) =>
            ...
            <Submenu
              ...
              itemProps={{
                ...
                handlers: {
                  onDragStart: (event: React.SyntheticEvent, index) =>
                    onDragStart({
