        ...
        if (bots.length > 0) {
            ...
            if (logs.length > 0) {
                const pickupCount = logs.filter(
                    (l) => l.reward >= 10, // RL_REWARD_PICKUP_ITEM = 10
                ).length;
                const outOfBoundsCount = logs.filter((l) => l.isOutOfBounds).length;
                const shootThroughWallCount = logs.filter(
                    (l) => l.shotThroughWall,
                ).length;
                const deathCount = logs.filter((l) => l.hp <= 0).length;
                const avgReward =
                    logs.reduce((sum, l) => sum + l.reward, 0) / logs.length;
